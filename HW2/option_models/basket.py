...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        cov_kk = cov_m[np.arange(n_assets), np.arange(n_assets)]
        cov_kk = cov_kk[:,None]
        #print(cov_kk)
        prices = forward[:,None] * np.exp(-0.5*texp*cov_kk + np.sqrt(texp) * chol_m @ znorm_m)
...
def basket_price_norm_analytic(
    ...
    vol = np.sqrt(weights @ cov_m @ weights[:,None])
    vol_std = np.fmax(vol * np.sqrt(texp), 1.0e-16)
 
    #print('vol_std:',vol_std)
